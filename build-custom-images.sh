...
# 构建方式选择
BUILD_MODE=${1:-"all"}  # all, api, web
ARCH=${2:-"arm64"}      # arm64 (客户服务器), amd64 (本地开发)

# 平台配置（支持客户ARM64服务器）
PLATFORM="linux/${ARCH}"
# 构建参数：第三个参数控制是否使用缓存
USE_CACHE=${3:-"false"}  # 默认不使用缓存（强制重建）
if [ "$USE_CACHE" = "true" ]; then
    ...
    echo -e "${BLUE}💨 使用Docker缓存加速构建${NC}"
else
    DOCKER_BUILD_ARGS="--platform ${PLATFORM} --build-arg HTTP_PROXY=http://127.0.0.1:10809 --build-arg HTTPS_PROXY=http://127.0.0.1:10809 --build-arg NO_PROXY=localhost,127.0.0.1 --no-cache --load"
    echo -e "${YELLOW}⚠️  强制重新构建（不使用缓存）${NC}"
fi
...
case ${BUILD_MODE} in
    ...
    *)
        ...
        echo "用法: $0 [构建模式] [架构] [使用缓存]"
        echo "构建模式: all, web, api"
        echo "架构: arm64 (默认), amd64"
        echo "使用缓存: false (默认，强制重建), true (使用缓存)"
        echo "示例: $0 web"
        echo "示例: $0 api"
        echo "示例: $0 api arm64 true  # 使用缓存构建API镜像"
        exit 1
        ;;
esac
...
if [ $TOTAL_SUCCESS -eq $TOTAL_ATTEMPTED ] && [ $TOTAL_SUCCESS -gt 0 ]; then
    ...
    if [ "$WEB_OPENSOURCE_BUILD_SUCCESS" = true ]; then
        ...
        if [ "$HOST_ARCH" = "x86_64" ] && [ "$ARCH" = "arm64" ]; then
            echo -e "${YELLOW}⚠️  跳过权限控制组件验证（架构不匹配：构建${ARCH}，宿主机${HOST_ARCH}）${NC}"
            echo -e "${BLUE}💡 在目标ARM64环境中运行以下命令验证：${NC}"
            echo "   docker run --rm --entrypoint=\"\" ${WEB_OPENSOURCE_IMAGE_NAME} test -f /app/web/components/route-guard/normal-user-guard.tsx"
        else
...
echo ""
echo -e "${BLUE}📖 构建选项：${NC}"
echo "  ./build-custom-images.sh              # 构建所有镜像（默认arm64，强制重建）"
echo "  ./build-custom-images.sh web          # 仅构建Web镜像"
echo "  ./build-custom-images.sh api          # 仅构建API镜像"
echo "  ./build-custom-images.sh api amd64    # 构建API镜像（amd64架构）"
echo "  ./build-custom-images.sh api arm64 true  # 使用缓存构建API镜像"
echo ""
